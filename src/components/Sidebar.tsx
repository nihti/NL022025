...
const Sidebar: React.FC<SidebarProps> = ({ openAddChartModal }) => {
  ...
  return (
    <>
      ...
      <AppBar position="static" sx={{ display: { xs: "flex", md: "none" }, backgroundColor: "#fff", padding: "6px" }}>
        <Toolbar sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          {/* Company Logo Placeholder */}
          {searchOpen ? 
          <img src={logoMobileSmall} alt="Company Logo" style={{ height: "32px" }} />
            : 
          <img src={logoMobileLarge} alt="Company Logo" style={{ height: "32px" }} />
          }
          <Box sx={{ display: "flex", alignItems: "center" }}>
            {/* Search Icon (Opens Search Field) */}
            { !searchOpen &&
            <IconButton onClick={() => setSearchOpen(!searchOpen)} sx={{ color: "#000", marginRight: 1 }}>
              <SearchIcon />
            </IconButton>
            }
      ...
      <Drawer anchor="right" open={menuOpen} onClose={() => setMenuOpen(false)} sx={{ 
        ...
        {/* Logo inside the menu */}
          ...
          {/* Close Button */}
              <IconButton onClick={() => setMenuOpen(false)} sx={{ float: "right", mb: 2 }}>
            <CloseIcon />
          </IconButton>
        </Box>
          ...
          {/* Add Chart Button */}
          <Button variant="contained" color="primary" fullWidth onClick={openAddChartModal} sx={{ mb: 2 }}>
            + Add Chart
          </Button>
        </Box>
      </Drawer>
      ...
      {editModalOpen && chartToEdit && (
        <ChartModal open={editModalOpen} onClose={() => setEditModalOpen(false)} isEditMode={true} initialChartData={chartToEdit} />
      )}
      ...
      >
        ...
        <List>
          {filteredCharts.length > 0 ? (
            filteredCharts.map((chart) => (
              <ListItem key={chart.id} sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                {/* Chart Name */}
                <ListItemText primary={chart.name} />
